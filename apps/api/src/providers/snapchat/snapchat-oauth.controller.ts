...
export class SnapchatOAuthController {
  constructor(private oauth: SnapchatOAuthService, private config: ConfigService) {}
  ...
  @ApiOperation({ summary: 'Snapchat OAuth redirect target. State JWT is the only auth. Redirects to /ad-accounts on success/error.' })
  ...
  async callback(
    ...
    try {
      ...
      res.redirect(302, oauthSuccessRedirect(this.config, 'SNAPCHAT', result.accountsConnected));
    } catch (err) {
      const message = err instanceof Error ? err.message : 'oauth_failed';
      res.redirect(302, oauthErrorRedirect(this.config, 'SNAPCHAT', message));
    }
  }
}
