...
import { TIKTOK_TOKEN_REFRESH_WINDOW_SECONDS } from './tiktok.constants';

// Token lifecycle:
//   - TikTok access tokens last ~24 hours.
//   - Refresh tokens are long-lived but persist only as long as the user
//     keeps the app authorized; they may rotate on each refresh and we
//     persist the new refresh_token when present.
//   - We refresh proactively when tokenExpiresAt falls within
//     TIKTOK_TOKEN_REFRESH_WINDOW_SECONDS of now (default: 30 minutes).
//   - On hard auth failure (401, code 40100), we mark the account ERROR
...
export class TikTokTokenService {
  ...
  async refreshIfNeeded(account: AdAccount): Promise<boolean> {
    if (account.platform !== Platform.TIKTOK) return false;
    if (account.status === AdAccountStatus.MOCK) return false;

    const cutoff = new Date(Date.now() + TIKTOK_TOKEN_REFRESH_WINDOW_SECONDS * 1000);
    const needsRefresh =
    ...
    try {
      ...
      if (!res.access_token) {
        throw new ProviderError(
          ...
          'TIKTOK',
          'TikTok returned no access token from refresh',
        );
      }
      const expiresAt = res.access_token_expire_in
        ? new Date(Date.now() + res.access_token_expire_in * 1000)
        : null;
      ...
      await this.prisma.adAccount.update({
        ...
        data: {
          ...
          // Persist a new refresh token only if TikTok returned one.
      ...
      this.logger.log(`TikTok token refreshed for adAccount=${account.id}`);
      return true;
    } catch (err) {
      const isAuthFail =
      ...
      if (isAuthFail) {
        ...
        this.logger.warn(`TikTok token irrecoverable for adAccount=${account.id}; reconnect required`);
      } else {
        // Transient — do not flip status, just log.
        this.logger.error(
          `TikTok token refresh transient error for adAccount=${account.id}: ${(err as Error).message}`,
        );
      }
      return false;
    }
